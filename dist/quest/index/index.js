  ...
  value: function (e) {
    for (key in e) {
      ...
      if (key == "current" && model) {
        ...
        $(".description").html(
          model.title +
            (newflag ? '<img src="https://pic2.zhimg.com/v2-894298321368004931eaecaa4000c7a1_r.gif" alt="">' : "")
        )
        ...
        e.pre.forEach((ep) => {
          for (i in kcpre) {
            if (trim(kcpre[i].code) == ep) {
              let num = Number(i)
              if (e.prerequisite.indexOf(num) == -1) {
                e.prerequisite.push(num)
              }
              break
            }
          }
...
function addreq(req) {
  ...
  if (req.category == "simple") {
    reqcate = categories[slideq.category].name
  }
  let times = req.times || req.amount || 1
  let map = req.map
  if (map && typeof map != "string") {
    map.forEach(function (m) {
      slides += reqcate + ": " + m + slidehtml(times)
    })
  } else if (req[app.slideact[req.category]]) {
    req[app.slideact[req.category]].forEach(function (m) {
      if (app.slideact[req.category] == "scraps") {
        reqcate = "废弃"
      }
      slides += reqcate + ": " + ifnull(m.id || m.name, "") + slidehtml(m.times || m.amount || 1)
    })
  } else {
    slides += reqcate + ": " + ifnull(req.map, "") + slidehtml(times)
  }
}
...
function update() {
  ...
  for (let wkid in app.valido) {
    for (let dt in app.valido[wkid]) {
      ...
      if (dt == app.beginutc(wkid, new Date())) setwk(wkid, "last", app.valido[wkid][app.beginutc(wkid, new Date())])
    }
  }
...
function setchart() {
  ...
  while (cl--) {
    ...
    if (switches.fs[0] && switches.fs[1] && switches.fs[2]) {
    } else {
      let status = false
